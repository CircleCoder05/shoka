      ...
      <main :class="['studio-main', { 'taxonomy-page': section === 'taxonomy', 'posts-page': section === 'posts' }]">
          ...
          <section class="studio-card post-manager">
            ...
            <div class="content-table">
              ...
              <article v-for="post in paginatedPosts" :key="post.id">
                ...
                <div class="table-actions"><button class="icon-action" title="编辑文章" aria-label="编辑文章" @click="editPost(post)">✎</button><button v-if="post.status !== 'published'" title="发布文章" aria-label="发布文章" @click="publishPost(post)">↑</button><button class="danger icon-action" title="删除文章" aria-label="删除文章" @click="removePost(post)">×</button></div>
              </article>
            </div>
        ...
        <template v-else-if="section === 'profile'">
          <form class="profile-admin" @submit.prevent="saveProfile">
            ...
            <section class="studio-card profile-admin-card"><h2>能力偏好</h2><div class="repeat-list"><label v-for="(item,index) in profileForm.skills" :key="index"><input v-model="item.name" placeholder="能力名称" /><input v-model.number="item.value" type="number" min="0" max="100" /><button type="button" class="danger icon-action" title="删除能力" aria-label="删除能力" @click="profileForm.skills.splice(index,1)">×</button></label><el-button @click="profileForm.skills.push({name:'',value:80})">＋ 添加能力</el-button></div></section>
            <section class="studio-card profile-admin-card"><h2>人生时间线</h2><div class="repeat-list"><label v-for="(item,index) in profileForm.timeline" :key="index"><input v-model="item.title" placeholder="阶段标题" /><input v-model="item.description" placeholder="阶段描述" /><button type="button" class="danger icon-action" title="删除阶段" aria-label="删除阶段" @click="profileForm.timeline.splice(index,1)">×</button></label><el-button @click="profileForm.timeline.push({title:'',description:''})">＋ 添加阶段</el-button></div></section>
            <section class="studio-card profile-admin-card profile-snapshots"><header><div><h2>理想生活碎片</h2><p>使用 9:16 竖图，每张图片只配一个标题。</p></div><el-upload :show-file-list="false" accept="image/*" :http-request="uploadProfileSnapshot"><el-button type="primary">＋ 上传图片</el-button></el-upload></header><div><article v-for="(item,index) in profileForm.snapshots" :key="item.url"><img :src="item.url" alt="" /><input v-model="item.title" placeholder="碎片标题" /><button type="button" class="danger snapshot-remove" title="删除图片" aria-label="删除图片" @click="profileForm.snapshots.splice(index,1)">×</button></article></div></section>
