...
本文系统展示了施瓦西时空中多种引力效应的MATLAB可视化实现，涵盖五个核心主题：一是星光偏折方程的解析与数值求解，通过动画演示不同质量对光线轨迹的影响；二是引力红移的可视化，分别展示观察者位置变化和质量变化时的红移分布；三是径向类光测地线的解析与数值求解，绘制内向族和外向族测地线族；四是黑洞阴影与吸积盘成像模拟，通过光线追踪算法生成施瓦西黑洞的模拟观测图像；五是施瓦西时空的嵌入图，包括静态星体嵌入图、随参数变化的动画演示以及最大延拓时空T=0时刻的嵌入图。所有程序均提供完整MATLAB代码，可生成动态可视化效果。
